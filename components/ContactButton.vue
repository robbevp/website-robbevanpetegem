    ...
    >
      ...
      >
        <EmailIcon
          ...
          class="icon p-3 rounded-full md:transition-all md:delay-1200 md:duration-200 md:hover:delay-0 md:focus:delay-0 md:ease-out"
          width="36"
          height="36"
        />
        <LinkedinIcon
          v-else-if="info.type === 'linkedin'"
          class="icon p-3 rounded-full md:transition-all md:delay-1200 md:duration-200 md:hover:delay-0 md:focus:delay-0 md:ease-out"
          width="36"
          height="36"
        />
        ...
        />
        <span
          class="my-auto pl-2 pr-5 lg:whitespace-no-wrap hidden md:block"
          :class="{ 'whitespace-no-wrap': info.text.length < 20 }"
        >
          {{ info.text }}
        </span>
      </button>
...
export default {
  ...
  props: {
    info: {
      ...
      required: true,
      validator(value) {
        return ["email", "linkedin", "phone"].includes(value.type);
      },
    },
  },
  computed: {
